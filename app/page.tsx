...
export default function Home() {
  ...
  function confirmClientTool(
    tool: "askForConfirmation" | "getBrowserLocation",
    toolCallId: string,
    output: unknown,
  ) {
    addToolOutput({
    ...
    if (isToolUIPart(part)) {
      return (
        <ToolPart
          ...
          onConfirmClientTool={confirmClientTool}
          onDenyClientTool={denyClientTool}
        />
      );
    }
              ...
              <div className="flex flex-1 items-center justify-center py-16">
                <div className="max-w-md text-center">
                  ...
                  <p className="mt-3 text-sm leading-6 text-zinc-500">
                    例如：查上海天气、获取我的浏览器位置、帮我确认是否继续、创建一张 3 个键盘预算 300 美元的采购单。
                  </p>
                </div>
              </div>
            ) : (
...
function ToolPart({
  ...
  return (
    <div className="rounded-md border border-zinc-200 bg-white p-3 text-sm text-zinc-800">
      ...
      {part.state === "input-available" && toolName === "askForConfirmation" ? (
        <div className="mt-3">
          <p className="text-zinc-700">{getString(part.input, "message")}</p>
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <button
              className="h-9 rounded-md bg-zinc-950 px-3 text-sm font-medium text-white transition-colors hover:bg-zinc-800"
              type="button"
              onClick={() =>
                onConfirmClientTool("askForConfirmation", part.toolCallId, {
                  confirmed: true,
                  message: getString(part.input, "message"),
                })
              }
            >
              确认
      ...
      {part.state === "input-available" && toolName === "getBrowserLocation" ? (
        <div className="mt-3">
          <p className="text-zinc-700">{getString(part.input, "reason")}</p>
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <button
              className="h-9 rounded-md bg-zinc-950 px-3 text-sm font-medium text-white transition-colors hover:bg-zinc-800"
              type="button"
              onClick={() =>
                onConfirmClientTool("getBrowserLocation", part.toolCallId, {
                  city: "Shanghai",
                  country: "China",
                  source: "frontend-mock-location",
                })
              }
            >
              允许
...
const toolLabels: Record<string, string> = {
  getWeatherInformation: "服务端天气工具",
  askForConfirmation: "前端确认工具",
  getBrowserLocation: "前端位置工具",
  createPurchaseOrder: "需 approval 的采购工具",
};
